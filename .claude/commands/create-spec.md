...
Next steps:
  ...
  2. Run: claw-forge validate-spec --strict-shape app_spec.txt
  ...
  4. Run: claw-forge run --concurrency 5
   ...
   The --strict-shape flag promotes Layer 4 shape WARNINGs (Gaps 3, 5, 6)
   to ERRORs. /create-spec emits shape-annotated <feature> elements
   throughout, so the spec is expected to pass --strict-shape on first
   run; if it doesn't, /fix-spec will auto-repair the failing elements.
