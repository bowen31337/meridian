...
For each epic in the extracted spec, generate a `<category>` block of
shape-annotated `<feature>` elements and show it. Default each feature to
`shape="plugin"` with a slug derived from the epic name (lowercase, non-
alphanumerics → dashes, collapse repeats). Use `shape="core"` with explicit
`touches_files` only for stories that are clearly cross-cutting (middleware,
error handlers, design tokens, shared types).
...
<category name="Authentication">
  ...
  <feature shape="plugin" plugin="authentication">
    <description>User can log in and receive a JWT access_token (returns 200)</description>
  </feature>
  <!-- ... -->
  <!-- For middleware-shaped stories, switch to shape="core": -->
  <feature shape="core" touches_files="src/api/middleware/jwt_auth.py">
    <description>API rejects requests with missing or expired JWT (returns 401)</description>
  </feature>
</category>
```

Do NOT add `index=` attributes — the parser derives them positionally.

Ask: "Approve this category (**[epic name]**, [N] features)? [approve/edit/skip]"

Process all epics one by one. Collect all approved/skipped `<category>` blocks into
the final `<core_features>` section.

> **Why shape annotations?** They let `claw-forge run` dispatch plugin
> features in parallel without merge conflicts (their `touches_files`
> are disjoint by construction) and serialize core features so two
> middleware rewrites can't race on the same file. Imported specs that
> emit shape annotations from the start validate cleanly under
> `claw-forge validate-spec --strict-shape`.
...
Next steps:
  ...
  2. claw-forge validate-spec --strict-shape <filename>
     Issues? Run /fix-spec, then re-run validate-spec until clean
  3. claw-forge plan <filename>
  4. claw-forge run --concurrency 5
```

> The `--strict-shape` flag promotes shape WARNINGs (Gaps 3, 5, 6) to
> ERRORs. Since `/import-spec` emits shape-annotated `<feature>` elements
> from every category, the imported spec should pass strict-shape on the
> first try. If it doesn't, run `/fix-spec` to repair the offending
> elements.
