"""
Shared test fixtures for the sdk-capabilities test suite.

OTel is mocked with a lightweight MockSpan / MockTracer pair that captures
...
import pytest
from sdk_capabilities._audit import AuditLog, AuditLogEntry
...
@pytest.fixture()
def mock_authorize_tracer(monkeypatch: pytest.MonkeyPatch) -> MockTracer:
    tracer = MockTracer()
    monkeypatch.setattr("sdk_capabilities._authorize.get_tracer", lambda: tracer)
    return tracer


@pytest.fixture()
def mock_authorize_span(mock_authorize_tracer: MockTracer) -> MockSpan:
    return mock_authorize_tracer.span
