...
def make_agent(id: str = "a1") -> Agent:
    now = _ts()
    return Agent(id=id, kind="meridian.research", name="Test Agent",
                 config="{}", capabilities="[]", created_at=now, updated_at=now)


def make_session(id: str = "s1", agent_id: str = "a1") -> Session:
    now = _ts()
    return Session(id=id, agent_id=agent_id, status="active",
                   metadata=None, created_at=now, updated_at=now)


def make_thread(id: str = "t1", session_id: str = "s1") -> Thread:
    now = _ts()
    return Thread(id=id, session_id=session_id, title="Hello",
                  created_at=now, updated_at=now)


def make_message(id: str = "m1", thread_id: str = "t1", session_id: str = "s1") -> Message:
    return Message(id=id, thread_id=thread_id, session_id=session_id,
                   role="user", content="[]", sequence=0, created_at=_ts())


def make_tool_call(id: str = "tc1", message_id: str = "m1", session_id: str = "s1") -> ToolCall:
    now = _ts()
    return ToolCall(id=id, message_id=message_id, session_id=session_id,
                    tool_name="search", input="{}", output=None,
                    status="pending", created_at=now, updated_at=now)


def make_skill(id: str = "sk1") -> Skill:
    now = _ts()
    return Skill(id=id, name="web_search", description="Search the web",
                 capabilities='["net.fetch"]', config="{}",
                 created_at=now, updated_at=now)


def make_environment(id: str = "e1") -> Environment:
    now = _ts()
    return Environment(id=id, kind="meridian.python", status="provisioned",
                       config="{}", created_at=now, updated_at=now)


def make_memory_entry(id: str = "me1", scope: str = "agent:a1", key: str = "k") -> MemoryEntry:
    now = _ts()
    return MemoryEntry(id=id, scope=scope, key=key, value="val",
                       created_at=now, updated_at=now)


def make_vault_entry(id: str = "v1") -> VaultEntry:
    now = _ts()
    return VaultEntry(id=id, name="openai/api_key", description="OpenAI key",
                      created_at=now, updated_at=now)


def make_user_profile(id: str = "u1") -> UserProfile:
    now = _ts()
    return UserProfile(id=id, username="alice", display_name="Alice",
                       email="alice@example.com", metadata=None,
                       created_at=now, updated_at=now)


def make_channel(id: str = "ch1") -> Channel:
    now = _ts()
    return Channel(id=id, kind="meridian.slack", name="general",
                   config="{}", status="active", created_at=now, updated_at=now)


def make_webhook(id: str = "wh1") -> Webhook:
    now = _ts()
    return Webhook(id=id, url="https://example.com/hook", events='["message.created"]',
                   secret_ref=None, status="active", created_at=now, updated_at=now)
...
class TestRuntimeGetSuccess:
    ...
    async def test_returns_none_for_missing(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
        rt = RepositoryRuntime(_StubDriver(), make_options(audit_log))
...
class TestRuntimeStoreRaises:
    async def test_wraps_as_repo_failure(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
        rt = RepositoryRuntime(_StubDriver(raises=RuntimeError("db error")), make_options(audit_log))
    ...
    async def test_entity_type_and_operation_set(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
    ...
    async def test_on_error_callback_invoked(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
        errors: list[RepositoryFailure] = []
        rt = RepositoryRuntime(_StubDriver(raises=RuntimeError("boom")), make_options(audit_log, errors))
        with pytest.raises(RepositoryFailure):
            await rt.agents.save(make_agent())
        assert len(errors) == 1
    ...
    async def test_get_wraps_as_get_failed(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
        rt = RepositoryRuntime(_StubDriver(raises=IOError("read fail")), make_options(audit_log))
        with pytest.raises(RepositoryFailure) as exc_info:
            await rt.agents.get("a1")
        assert exc_info.value.code == "REPO_GET_FAILED"

    async def test_delete_wraps_as_delete_failed(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
    ...
    async def test_list_wraps_as_list_failed(self, mock_span: MockSpan, audit_log: CapturingAuditLog) -> None:
        rt = RepositoryRuntime(_StubDriver(raises=RuntimeError("boom")), make_options(audit_log))
        with pytest.raises(RepositoryFailure) as exc_info:
...
class TestSqliteAgent:
    ...
    async def test_upsert_updates_record(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        now = _ts()
        agent = Agent(id="a1", kind="k1", name="Old", config="{}", capabilities="[]",
                      created_at=now, updated_at=now)
        await sqlite_driver.agents.save(agent)
        updated = Agent(id="a1", kind="k2", name="New", config="{}", capabilities="[]",
                        created_at=now, updated_at=_ts())
    ...
    async def test_list_filter_by_kind(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        now = _ts()
        await sqlite_driver.agents.save(Agent(id="a1", kind="research", name="R",
                                               config="{}", capabilities="[]",
                                               created_at=now, updated_at=now))
        await sqlite_driver.agents.save(Agent(id="a2", kind="coding", name="C",
                                               config="{}", capabilities="[]",
                                               created_at=now, updated_at=now))
...
class TestSqliteSession:
    ...
    async def test_list_filter_by_status(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        now = _ts()
        await sqlite_driver.sessions.save(Session(id="s1", agent_id="a1", status="active",
                                                   metadata=None, created_at=now, updated_at=now))
        await sqlite_driver.sessions.save(Session(id="s2", agent_id="a1", status="closed",
                                                   metadata=None, created_at=now, updated_at=now))
...
class TestSqliteMessage:
    ...
    async def test_list_ordered_by_sequence(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        for seq in (2, 0, 1):
            await sqlite_driver.messages.save(
                Message(id=f"m{seq}", thread_id="t1", session_id="s1",
                        role="user", content="[]", sequence=seq, created_at=now)
            )
    ...
    async def test_list_filter_by_role(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        now = _ts()
        await sqlite_driver.messages.save(
            Message(id="m1", thread_id="t1", session_id="s1", role="user",
                    content="[]", sequence=0, created_at=now)
        )
        await sqlite_driver.messages.save(
            Message(id="m2", thread_id="t1", session_id="s1", role="assistant",
                    content="[]", sequence=1, created_at=now)
        )
...
class TestSqliteToolCall:
    ...
    async def test_list_filter_by_status(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        now = _ts()
        pending = ToolCall(id="tc1", message_id="m1", session_id="s1", tool_name="search",
                           input="{}", output=None, status="pending", created_at=now, updated_at=now)
        done = ToolCall(id="tc2", message_id="m1", session_id="s1", tool_name="search",
                        input="{}", output="{}", status="success", created_at=now, updated_at=now)
    ...
    async def test_list_ordered_by_name(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        for name in ("z_skill", "a_skill", "m_skill"):
            ...
            await sqlite_driver.skills.save(
                Skill(id=s_id, name=name, description="",
                      capabilities="[]", config="{}", created_at=now, updated_at=now)
            )
...
class TestSqliteEnvironment:
    ...
    async def test_list_filter_by_kind(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        await sqlite_driver.environments.save(
            Environment(id="e1", kind="python", status="provisioned", config="{}", created_at=now, updated_at=now)
        )
        await sqlite_driver.environments.save(
            Environment(id="e2", kind="node", status="provisioned", config="{}", created_at=now, updated_at=now)
        )
...
class TestSqliteMemory:
    ...
    async def test_list_filter_by_scope(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        await sqlite_driver.memory.save(
            MemoryEntry(id="me1", scope="agent:a1", key="k1", value="v1", created_at=now, updated_at=now)
        )
        await sqlite_driver.memory.save(
            MemoryEntry(id="me2", scope="agent:a2", key="k1", value="v2", created_at=now, updated_at=now)
        )
    ...
    async def test_list_ordered_by_username(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        for username in ("zara", "alice", "mike"):
            await sqlite_driver.user_profiles.save(
                UserProfile(id=username, username=username, display_name=None,
                            email=None, metadata=None, created_at=now, updated_at=now)
            )
...
class TestSqliteChannel:
    ...
    async def test_list_filter_by_status(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        await sqlite_driver.channels.save(
            Channel(id="c1", kind="slack", name="general", config="{}", status="active",
                    created_at=now, updated_at=now)
        )
        await sqlite_driver.channels.save(
            Channel(id="c2", kind="slack", name="alerts", config="{}", status="inactive",
                    created_at=now, updated_at=now)
        )
...
class TestSqliteWebhook:
    ...
    async def test_list_filter_by_status(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        ...
        await sqlite_driver.webhooks.save(
            Webhook(id="wh1", url="https://a.com", events="[]", secret_ref=None,
                    status="active", created_at=now, updated_at=now)
        )
        await sqlite_driver.webhooks.save(
            Webhook(id="wh2", url="https://b.com", events="[]", secret_ref=None,
    ...
    async def test_secret_ref_nullable(self, sqlite_driver: SqliteRepositoryDriver) -> None:
        now = _ts()
        wh = Webhook(id="wh1", url="https://a.com", events="[]",
