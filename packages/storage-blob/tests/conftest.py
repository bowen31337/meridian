"""
Shared test fixtures for the blob-store conformance suite.
...
from storage_blob._audit import AuditLog
...
def mock_tracer(monkeypatch: pytest.MonkeyPatch) -> MockTracer:
    tracer = MockTracer()
    monkeypatch.setattr("storage_blob._runtime.get_tracer", lambda: tracer)
    return tracer
