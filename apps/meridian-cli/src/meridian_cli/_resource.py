...
def _run(
    ...
    *,
    ...
    with tracer.start_as_current_span(
        span_name, attributes={"resource": resource, "operation": operation}
    ) as span:
        ...
        write_audit(
            ...
            f"{span_name}.invoked",
            {
                "resource": resource,
                "operation": operation,
                **{k: v for k, v in event_attrs.items() if isinstance(v, (str, int, float, bool))},
            },
        )
...
def make_crud_group(resource: str) -> click.Group:
    ...
    @grp.command("list", help=f"List all {resource}.")
    @click.pass_context
    def _list(ctx: click.Context) -> None:
        _run(resource, "list", "GET", path_base, ctx=ctx, event_attrs={})

    @grp.command(
        "get", help=f"Get a {resource[:-1] if resource.endswith('s') else resource} by ID."
    )
    @click.argument("id")
    @click.pass_context
    ...
    @grp.command(
        "create",
        help=f"Create a new {resource[:-1] if resource.endswith('s') else resource}.",
    )
    ...
    @grp.command(
        "update",
        help=f"Update an existing {resource[:-1] if resource.endswith('s') else resource}.",
    )
    @click.argument("id")
    ...
    @grp.command(
        "delete",
        help=f"Delete a {resource[:-1] if resource.endswith('s') else resource} by ID.",
    )
    @click.argument("id")
    @click.pass_context
